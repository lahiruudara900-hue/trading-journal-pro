...
export default function TradeDetailPage() {
  ...
  // Find a display name for this trade
  const tradeTitle = (() => {
  ...
  // Get result for badge color
  const resultField = fields.find(f => f.field_name.toLowerCase().includes('result'))
        ...
        <main className="flex-1 md:ml-56 pt-14 md:pt-0">
          <div style={{ padding: '1.5rem', maxWidth: '64rem', margin: '0 auto' }}>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', color: '#8888a0', padding: '3rem' }}>Trade not found.</div>
            ) : (
              <>
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.25rem' }}>
                      <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white' }}>{tradeTitle}</h1>
                      {resultVal && (
                        <span style={{
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                    <Link href={`/trades/${id}/edit`} className="btn-secondary" style={{ fontSize: '0.875rem' }}>Edit</Link>
                    <button onClick={handleDelete} className="btn-danger" style={{ fontSize: '0.875rem' }}>Delete</button>
                  </div>
                </div>
